...
test("云端发布按预检、迁移试跑、迁移、生产配置和必要初始化顺序执行", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(stdout + stderr, /--include-seed/);
...
test("云端发布 dry-run 也会在任何远端步骤前执行证据预检", async () => {
  const child = spawn(process.execPath, ["scripts/deploy-cloud.ts"], {
    cwd: projectDirectory,
    env: {
      PATH: process.env.PATH,
  ...
  assert.match(stderr, /缺少 RELEASE_EVIDENCE_DIR/);
  assert.doesNotMatch(stdout, /2\. Supabase 项目连接/);
});

async function createPrerequisiteEvidence() {
  const directory = await mkdtemp(
    join(tmpdir(), "groundeddesk-deploy-evidence-"),
  );
  await Promise.all(["local-gate", "live-ai-smoke"].map((check) =>
    writeFile(
